## Congressional Tweets - Twitter House
## Input: 
## Output: 
## c. Thu Feb 22 20:34:29 CST 2018
...
# houseProfile_tbl$profile_image_url <- gsub("_normal", "_400x400", houseProfile_tbl$profile_image_url)

#####################
## Download Tweets ##
#####################
n <- nrow(houseProfile_tbl)
...
for (i in 389:n) {
  ...
  # User information
  user_id <- gsub("x", "", houseProfile_tbl$user_id[i])
  user_name <- houseProfile_tbl$screen_name[i]
  user_party <- houseProfile_tbl$party[i]
  user_ideology <- houseProfile_tbl$govTrack_ideology[i]
  user_leadership <- houseProfile_tbl$govTrack_leadership[i]
  ...
  houseProfile_tbl$n_obs_timeline[i] <- n_obs
  houseProfile_tbl$n_obs_origContent[i] <- n_OC
  houseProfile_tbl$n_obs_textOnly[i] <- n_text
  houseProfile_tbl$prop_origContent[i] <- n_OC / n_obs
  houseProfile_tbl$prop_textOnly[i] <- n_text / n_obs
  
  ## Date range for full timeline
  houseProfile_tbl$earliest_status[i] <- as.character(user_timeline$created_at[n_obs])
  houseProfile_tbl$latest_status[i] <- as.character(user_timeline$created_at[1])
  
  ## Original Content summary statistics
  houseProfile_tbl$mean_favCount_OC[i] <- mean(original_content$favorite_count)
  houseProfile_tbl$median_favCount_OC[i] <- median(original_content$favorite_count)
  houseProfile_tbl$stdDev_favCount_OC[i] <- sd(original_content$favorite_count)
  houseProfile_tbl$mean_rtCount_OC[i] <- mean(original_content$retweet_count)
  houseProfile_tbl$median_rtCount_OC[i] <- median(original_content$retweet_count)
  houseProfile_tbl$stdDev_rtCount_OC[i] <- sd(original_content$retweet_count)
  
  ## Text Only summary statistics
  houseProfile_tbl$mean_favCount_text[i] <- mean(text_only$favorite_count)
  houseProfile_tbl$median_favCount_text[i] <- median(text_only$favorite_count)
  houseProfile_tbl$stdDev_favCount_text[i] <- sd(text_only$favorite_count)
  houseProfile_tbl$mean_rtCount_text[i] <- mean(text_only$retweet_count)
  houseProfile_tbl$median_rtCount_text[i] <- median(text_only$retweet_count)
  houseProfile_tbl$stdDev_rtCount_text[i] <- sd(text_only$retweet_count)
  ...
  ## WRITE DATA ##
  ################
  filename <- paste0(houseProfile_tbl$chamber[i], "_", 
                     houseProfile_tbl$party[i], "_", 
                     houseProfile_tbl$screen_name[i])
  ...
  image_link <- houseProfile_tbl$profile_image_url[i]
  download.file(image_link, paste0("Data/Twitter/", filename, "_profileImage.jpg"), mode = 'wb')
}

## Note the hiccup at index 388 (too few tweets for subsetting to text only and OC hashtags)

## Write data
write.csv(houseProfile_tbl, "Output/houseProfile_tbl_2018-02-23.csv", row.names = FALSE)
